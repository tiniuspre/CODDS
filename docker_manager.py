...
from images.program_handler import Process, Challenge, ImageHandler, PROGRAM_HANDLER_BASE_DIR
...
class ContainerGroup:
    def __init__(self, identifier: str, challenge: Challenge, pin_code: str = None):
        self.identifier = identifier
        self.challenge = challenge
        self.network = self.create_network()
        self.pin = pin_code
        self.processes = {
            process.dockerfile: DockerContainer(identifier, process, challenge.info.connection_type, self.network.id)
            for process in challenge.processes
        }
...
class DockerManager:
    ...
    def spawn(self, identifier: str, pin_code: str = None):
    ...
    def get_all_containers_dict(self):
...
class ManagerHandler:
    ...
    def get_manager(self, challenge_name) -> DockerManager:
...
if __name__ == '__main__':
    ...
    print(manager.get_container(f"test-{s_id}").__dict__())
